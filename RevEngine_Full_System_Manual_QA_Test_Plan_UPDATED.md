# Rev Engine — Full-System Manual QA Test Plan

**Scope:** Every module of the Rev Engine platform — Express/Prisma/PostgreSQL backend, owner signup and onboarding, mock SaaS plan selection/subscription management, company-member invitations, company-created saved roles, granular permissions and access scopes, static admin web UI, client portal, public booking/tracking portal, regional Zimbabwe/South Africa local deployments, customer payment experience, regional payment-provider setup, finance/accounting exports, business-performance analytics, and the Flutter technician mobile app.
...
- Native browser `alert()`, `confirm()`, and `prompt()` are forbidden. Use the shared Rev Engine notification system for success/failure and a proper modal for confirmation or extra input.
...
```bash
grep -n '^DATABASE_URL\|^PORT\|^REVENGINE_REGION' .env .env.zw .env.sa
...
```text
.env:DATABASE_URL="postgresql://<real-local-user>:<real-local-password>@localhost:5432/revengine..."
.env.zw:DATABASE_URL="postgresql://<same-real-user>:<same-real-password>@localhost:5432/revengine_zw..."
.env.zw:REVENGINE_REGION="ZW"
.env.zw:PORT=3000
.env.sa:DATABASE_URL="postgresql://<same-real-user>:<same-real-password>@localhost:5432/revengine_sa..."
.env.sa:REVENGINE_REGION="SA"
.env.sa:PORT=3001
```
...
sudo -u postgres dropdb --if-exists revengine_zw
sudo -u postgres dropdb --if-exists revengine_sa
sudo -u postgres createdb -O "$DB_OWNER" revengine_zw
sudo -u postgres createdb -O "$DB_OWNER" revengine_sa
...
- `revengine_zw` contains Zimbabwe seeded accounts only.
- `revengine_sa` contains South Africa seeded accounts only.
...
- [ ] **Rev Engine Subscription** appears only for the owner or a non-owner explicitly marked Full Access with company-wide scope.
...
| ENV-04 | For a fresh full regression only: create/reset `revengine_zw` and `revengine_sa`, then run `npm run db:reset:zw` and `npm run db:reset:sa` | Clean databases are reset and seeded without manual Prisma migration prompts | P0 | F |
| ENV-05 | `npm test` | All automated tests pass with 0 failures before manual QA starts | P0 | F |
| ENV-06 | `npm run dev:zw`, hit `http://localhost:3000/healthz` and `/readyz` | ZW server is healthy and connected to `revengine_zw` | P0 | R |
| ENV-07 | `npm run dev:sa`, hit `http://localhost:3001/healthz` and `/readyz` | SA server is healthy and connected to `revengine_sa` | P0 | R |
...
Rev Engine must separate these ideas:
...
| AUTH-25 | Invite an email already belonging to an existing Rev Engine user | Existing account is not hijacked or silently rebound to another company; behavior follows explicit safe membership rules | P0 | S |
...
| ROLE-02 | Create a role from scratch in a company with no saved roles | No fixed generic Rev Engine role is forced into the UI | P0 | F |
...
| MEM-04 | Create a new role while inviting and choose exact permissions | The role can be saved for reuse; no generic Rev Engine roles are forced on the company | P0 | F |
...
## 9. Invoicing, Customer Payments & Rev Engine Subscription
...
This setup is for **the business's customers paying that business**. It is separate from businesses paying Rev Engine for their subscription.
...
### 9.3 Rev Engine subscription

| ID | Steps | Expected Result | Priority | Type |
|---|---|---|---|---|
| BILL-01 | Open profile dropdown as owner | Rev Engine Subscription appears and loads | P0 | F |
...
| ONB-06 | Open the top-right account menu across authenticated pages | Settings, Rev Engine Subscription, Security, and Log out are present according to permission/ownership rules; old duplicate sidebar account treatment is not competing | P1 | F |
...
| UX-02 | Perform create, update, send, revoke, delete and failure actions across modules | Each action gives standard success/failure notification; confirmations use Rev Engine modal | P0 | F |
...
- [ ] Rev Engine Subscription is visible only to OWNER or explicit Full Access + COMPANY scope.
...
- [ ] For a fresh full regression, `revengine_zw` and `revengine_sa` were reset cleanly; for a resumed run, existing QA data was intentionally preserved.
...
- [ ] Top-right account menu works: Settings, Security and Log out for all authorized accounts; Rev Engine Subscription follows the explicit Full Access rule.
...
|---|---|
| Regional databases | 2 separate DBs: `revengine_zw`, `revengine_sa` |
