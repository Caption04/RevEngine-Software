# Rev Engine Regional Local Development
...
```text
.env.zw -> revengine_zw, PORT=3000, REVENGINE_REGION=ZW
.env.sa -> revengine_sa, PORT=3001, REVENGINE_REGION=SA
```
...
`db:reset:zw` loads `.env.zw`, force-resets `revengine_zw`, and seeds only Zimbabwe.
`db:reset:sa` loads `.env.sa`, force-resets `revengine_sa`, and seeds only South Africa.
...
```bash
grep -n '^DATABASE_URL\|^PORT\|^REVENGINE_REGION' .env.zw .env.sa
```

The database names should be `revengine_zw` and `revengine_sa`, with ports `3000` and `3001`.
