        ...
        child: SafeArea(
          child: ListView(
            ...
            children: <Widget>[
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        const Text('Offline queue and device registration', style: TextStyle(color: RevEnginePalette.muted)),
                      ],
                    ),
                  ),
                ],
              ...
              PremiumCard(
                glowColor: actions.isEmpty ? RevEnginePalette.success : RevEnginePalette.warning,
                child: Row(
                  children: <Widget>[
                    PremiumIconTile(icon: actions.isEmpty ? Icons.cloud_done_outlined : Icons.cloud_sync_outlined, color: actions.isEmpty ? RevEnginePalette.success : RevEnginePalette.warning),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          Text('Device: ${widget.apiClient.deviceId ?? 'not registered'}', style: const TextStyle(color: RevEnginePalette.muted)),
                        ],
                      ),
                    ),
                  ],
              ...
              if (_error != null) ...<Widget>[
                const SizedBox(height: 14),
                PremiumCard(
                  glowColor: RevEnginePalette.danger,
                  child: Row(
                    children: <Widget>[
                      const Icon(Icons.error_outline, color: RevEnginePalette.danger),
                      const SizedBox(width: 10),
                      Expanded(child: Text(_error!, style: const TextStyle(color: RevEnginePalette.text))),
                    ],
                  ),
                ),
              ],
              ...
              if (actions.isEmpty)
                const PremiumCard(
                  child: Text('No queued actions. You are fully synced.', style: TextStyle(color: RevEnginePalette.muted)),
                )
              else
                ...actions.map(
                  (action) => Padding(
                    ...
                    child: PremiumCard(
                      ...
                      child: Row(
                        children: <Widget>[
                          const PremiumIconTile(icon: Icons.pending_actions_outlined, color: RevEnginePalette.warning),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: <Widget>[
                                ...
                                const SizedBox(height: 4),
                                Text(action.clientActionId, style: const TextStyle(color: RevEnginePalette.muted, fontSize: 12)),
                              ],
                            ),
                          ),
                          Text(_time(action.queuedAt), style: const TextStyle(color: RevEnginePalette.muted, fontWeight: FontWeight.w700)),
                        ],
                      ),
                    ),
                  ),
              ...
              if (_lastResults.isEmpty)
                const PremiumCard(child: Text('No sync results yet.', style: TextStyle(color: RevEnginePalette.muted)))
              else
                ..._lastResults.map(
                  (result) => Padding(
                    padding: const EdgeInsets.only(bottom: 10),
                    child: PremiumCard(
                      ...
                      child: ListTile(
                        ...
                        leading: const Icon(Icons.fact_check_outlined, color: RevEnginePalette.primaryBright),
                        title: Text('${result['actionType'] ?? 'Action'} — ${result['status'] ?? 'UNKNOWN'}'),
                        subtitle: Text((result['error'] ?? result['message'] ?? result['clientActionId'] ?? '').toString(), style: const TextStyle(color: RevEnginePalette.muted)),
                      ),
                    ),
                  ),
                ),
