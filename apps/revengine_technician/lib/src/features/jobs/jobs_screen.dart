...
class _JobsScreenState extends State<JobsScreen> {
  ...
  List<RevEngineJob> _jobs = <RevEngineJob>[];
  bool _loading = true;
  String? _error;
  String _query = '';
  ...
  Future<void> _openJob(RevEngineJob job) async {
    await Navigator.of(context).push(MaterialPageRoute<void>(
      ...
      body: PremiumBackground(
        child: SafeArea(
          child: RefreshIndicator(
            onRefresh: _pullJobs,
            color: RevEnginePalette.primaryBright,
            backgroundColor: RevEnginePalette.panel,
            child: _body(jobs),
          ),
        ),
      ),
  ...
  Widget _body(List<RevEngineJob> jobs) {
    if (_loading) {
  ...
  List<RevEngineJob> get _filteredJobs {
        ...
        Expanded(
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(_todayLabel(), style: const TextStyle(color: RevEnginePalette.muted)),
            ],
          ),
        ),
          ...
          children: <Widget>[
            ...
            if (pendingCount > 0)
              Container(
                ...
                decoration: BoxDecoration(color: RevEnginePalette.danger, borderRadius: BorderRadius.circular(999)),
...
class _StatsRow extends StatelessWidget {
  ...
  final List<RevEngineJob> jobs;

  @override
  Widget build(BuildContext context) {
    return Row(
      children: <Widget>[
        Expanded(child: _StatCard(icon: Icons.assignment_ind_outlined, label: 'Assigned', value: jobs.length, color: RevEnginePalette.purple)),
        const SizedBox(width: 10),
        Expanded(child: _StatCard(icon: Icons.play_circle_outline, label: 'In Progress', value: _count('PROGRESS'), color: RevEnginePalette.primaryBright)),
        const SizedBox(width: 10),
        Expanded(child: _StatCard(icon: Icons.check_circle_outline, label: 'Done', value: _count('COMPLETE'), color: RevEnginePalette.success)),
      ],
    );
  }
...
class _JobCard extends StatelessWidget {
          ...
          Expanded(
            child: Column(
              ...
              children: <Widget>[
                Row(
                  children: <Widget>[
                    Expanded(child: Text(job.id.isEmpty ? 'JOB' : job.id, style: const TextStyle(color: RevEnginePalette.muted, fontSize: 12, fontWeight: FontWeight.w700))),
  ...
  Color _serviceColor(String value) {
    final text = value.toLowerCase();
    if (text.contains('repair')) return RevEnginePalette.danger;
    if (text.contains('inspect')) return RevEnginePalette.warning;
    if (text.contains('clean')) return RevEnginePalette.cyan;
    return RevEnginePalette.primary;
  }
...
class _MetaLine extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: <Widget>[
          Icon(icon, color: RevEnginePalette.muted, size: 14),
          const SizedBox(width: 6),
          Expanded(child: Text(text, style: const TextStyle(color: RevEnginePalette.muted, fontSize: 12), maxLines: 1, overflow: TextOverflow.ellipsis)),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        color: RevEnginePalette.midnight.withValues(alpha: 0.96),
        border: Border(top: BorderSide(color: RevEnginePalette.border.withValues(alpha: 0.5))),
      ),
      child: SafeArea(
        child: Padding(
...
class _BottomNavItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = active ? RevEnginePalette.primaryBright : RevEnginePalette.muted;
    return InkWell(
