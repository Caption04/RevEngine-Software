...
import '../../core/models/revengine_job.dart';
...
class JobDetailScreen extends StatefulWidget {
  ...
  final RevEngineApiClient apiClient;
  final OfflineQueueRepository offlineQueue;
  final RevEngineJob job;
...
class _JobDetailScreenState extends State<JobDetailScreen> {
  ...
  Future<void> _queueProofPhoto() async {
    ...
    } on RevEngineApiException catch (error) {
      if (!mounted) return;
  ...
  Future<void> _sendCurrentLocation() async {
    ...
    try {
      final location = await RevEngineLocationService.currentPosition();
      try {
    ...
    } on RevEngineLocationException catch (error) {
      if (!mounted) return;
      setState(() {
        _locationMessage = error.message;
        _locationQueued = false;
  ...
  Future<void> _queueLocationForSync(RevEngineLocationReading location) async {
    await widget.offlineQueue.enqueue(
                ...
                PremiumCard(
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Text(description, style: const TextStyle(color: RevEnginePalette.muted, height: 1.45)),
                    ],
                  ),
                ),
              ],
        ...
        Expanded(
          child: Column(
            ...
            children: <Widget>[
              ...
              Text('Job Details', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: RevEnginePalette.muted)),
            ],
          ),
        ),
...
class _HeroCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PremiumCard(
      glowColor: RevEnginePalette.primary,
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          const PremiumIconTile(icon: Icons.work_outline, color: RevEnginePalette.primaryBright),
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                if ((job.serviceName ?? '').isNotEmpty) ...<Widget>[
                  ...
                  Text(job.serviceName!, style: const TextStyle(color: RevEnginePalette.primaryBright, fontWeight: FontWeight.w700)),
                ],
        ...
        children: <Widget>[
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: const LinearGradient(colors: <Color>[RevEnginePalette.primaryBright, RevEnginePalette.primary]),
              boxShadow: <BoxShadow>[BoxShadow(color: RevEnginePalette.primary.withValues(alpha: 0.28), blurRadius: 22)],
            ),
            child: Center(
          ...
          Expanded(
            child: Column(
              ...
              children: <Widget>[
                ...
                Text(phone ?? 'Contact number unavailable', style: const TextStyle(color: RevEnginePalette.muted)),
              ],
            ),
          ),
...
class _InfoRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: <Widget>[
        Icon(icon, color: RevEnginePalette.primaryBright, size: 20),
        const SizedBox(width: 12),
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[
              Text(label, style: const TextStyle(color: RevEnginePalette.muted, fontSize: 12)),
...
class _LocationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusColor = queued ? RevEnginePalette.warning : RevEnginePalette.muted;
    return PremiumCard(
      glowColor: queued ? RevEnginePalette.warning : RevEnginePalette.cyan,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Row(
            children: <Widget>[
              const PremiumIconTile(icon: Icons.my_location_outlined, color: RevEnginePalette.cyan),
              const SizedBox(width: 14),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
        ...
        children: <Widget>[
          Row(
            children: <Widget>[
              ...
              const Spacer(),
              Text('$completed of 6', style: const TextStyle(color: RevEnginePalette.muted, fontWeight: FontWeight.w700)),
            ],
          ),
          const SizedBox(height: 12),
          ClipRRect(
            ...
            child: LinearProgressIndicator(
              ...
              valueColor: const AlwaysStoppedAnimation<Color>(RevEnginePalette.primaryBright),
            ),
          ),
          const SizedBox(height: 14),
          ...
          if (pendingActions > 0) ...<Widget>[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                color: RevEnginePalette.warning.withValues(alpha: 0.1),
                border: Border.all(color: RevEnginePalette.warning.withValues(alpha: 0.26)),
              ),
              child: Row(
                children: <Widget>[
                  const Icon(Icons.sync_problem, color: RevEnginePalette.warning, size: 18),
                  const SizedBox(width: 10),
                  Expanded(child: Text('$pendingActions offline action(s) waiting to sync.', style: const TextStyle(color: RevEnginePalette.muted))),
                ],
              ),
            ),
          ],
...
class _ProgressStep extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: <Widget>[
          Icon(done ? Icons.check_circle : Icons.radio_button_unchecked, color: done ? RevEnginePalette.primaryBright : RevEnginePalette.muted, size: 20),
          const SizedBox(width: 10),
          Expanded(child: Text(label, style: TextStyle(color: done ? RevEnginePalette.text : RevEnginePalette.muted))),
          const Icon(Icons.chevron_right, color: RevEnginePalette.muted, size: 18),
        ],
      ),
    );
  }
    ...
    return PremiumCard(
      ...
      child: Column(
        children: <Widget>[
          ...
          _divider(),
          _ActionTile(icon: Icons.report_problem_outlined, title: 'Customer unavailable', subtitle: 'Queue unavailable status note', onTap: onCustomerUnavailable, color: RevEnginePalette.warning),
        ],
      ),
    );
  }

  Widget _divider() => Divider(height: 1, color: RevEnginePalette.border.withValues(alpha: 0.45));
}

class _ActionTile extends StatelessWidget {
  const _ActionTile({required this.icon, required this.title, required this.subtitle, required this.onTap, this.color = RevEnginePalette.primaryBright});
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      subtitle: Text(subtitle, style: const TextStyle(color: RevEnginePalette.muted)),
      trailing: const Icon(Icons.chevron_right, color: RevEnginePalette.muted),
      onTap: onTap,
    );
  }
}
...
class _SectionLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: <Widget>[
        Icon(icon, color: RevEnginePalette.primaryBright, size: 18),
...
class _MetaLine extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: <Widget>[
        Icon(icon, color: RevEnginePalette.muted, size: 16),
        const SizedBox(width: 8),
        Expanded(child: Text(text, style: const TextStyle(color: RevEnginePalette.muted))),
      ],
    );
  }
}
