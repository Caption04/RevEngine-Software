...
import '../models/revengine_job.dart';
import '../offline/offline_queue.dart';

class RevEngineApiException implements Exception {
  const RevEngineApiException(this.message, {this.statusCode});

  final String message;
  final int? statusCode;

  @override
  String toString() => 'RevEngineApiException($statusCode): $message';
}

class RevEngineApiClient {
  RevEngineApiClient({http.Client? httpClient}) : _httpClient = httpClient ?? http.Client();

  static const _baseUrlKey = 'revengine.api.baseUrl';
  static const _cookieKey = 'revengine.api.cookie';
  static const _deviceIdKey = 'revengine.device.id';

  final http.Client _httpClient;
  String _baseUrl = RevEngineEnvironment.defaultApiBaseUrl;
  String? _cookie;
  String? _deviceId;
  ...
  Future<void> loadSession() async {
    ...
    _baseUrl = prefs.getString(_baseUrlKey) ?? RevEngineEnvironment.defaultApiBaseUrl;
  ...
  Future<void> saveBaseUrl(String value) async {
    ...
    if (sanitized.isEmpty) throw const RevEngineApiException('API base URL is required');
    final prefs = await SharedPreferences.getInstance();
    _baseUrl = sanitized;
  ...
  Future<Map<String, dynamic>> login({
    ...
    if (data['twoFactorRequired'] == true) {
      throw const RevEngineApiException('Two-factor verification is required. Use a WORKER/technician account in this app.');
    }

    final role = data['role']?.toString().toUpperCase();
    if (role != null && role != 'WORKER') {
      throw const RevEngineApiException('This app only accepts WORKER/technician accounts. Use the admin web dashboard for owner/admin accounts.');
    }

    final setCookie = response.headers['set-cookie'];
    final authCookieMatch = setCookie == null
        ? null
        : RegExp(r'((?:__Host-)?revengine_token=[^;,]+)').firstMatch(setCookie);

    if (authCookieMatch == null) {
      throw const RevEngineApiException('Login did not return a technician session cookie. Confirm you are using the local API URL and a WORKER account.');
    }
  ...
  Future<Map<String, dynamic>> registerDevice() async {
    _deviceId ??= 'revengine-install-${DateTime.now().microsecondsSinceEpoch}';
    final prefs = await SharedPreferences.getInstance();
    ...
    final response = await _post('/api/worker/devices/register', <String, dynamic>{
      ...
      'deviceId': _deviceId,
      'deviceName': 'Rev Engine Technician',
      'deviceModel': _deviceModel(),
      'appVersion': RevEngineEnvironment.appVersion,
    });
  ...
  Future<List<RevEngineJob>> pullJobs({DateTime? since}) async {
    ...
    if (jobs is! List) return <RevEngineJob>[];
    return jobs.whereType<Map>().map((job) => RevEngineJob.fromJson(job.cast<String, dynamic>())).toList();
  }
  ...
  Map<String, dynamic> _decodeData(http.Response response) {
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      ...
      throw RevEngineApiException(message ?? 'Request failed', statusCode: response.statusCode);
    }
  ...
  String _requireDevice() {
    ...
    if (value == null || value.isEmpty) throw const RevEngineApiException('Device is not registered');
    return value;
  }
